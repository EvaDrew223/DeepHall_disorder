...
class LogManager:
    ...
    def save_disorder_config(self, impurity_config) -> None:
        """Save disorder configuration to file for reproducibility.

        Args:
            impurity_config: ImpurityConfiguration to save.
        """
        disorder_path = self.save_path / "disorder_config.npz"
        logger.info("Saving disorder configuration to %s", disorder_path)
        with disorder_path.open("wb") as f:
            np.savez(
                f,
                n_dis=impurity_config.n_dis,
                positions=np.array(impurity_config.positions),
                charges=np.array(impurity_config.charges),
