...
from deephall.types import ImpurityConfiguration, LogPsiNetwork, TrainingInit, TrainingStep
...
def make_optimizer_step(
    cfg: Config, network: LogPsiNetwork, impurity_config: ImpurityConfiguration | None = None
) -> tuple[TrainingInit, TrainingStep]:
    loss_grad_fn = make_loss_fn(network, cfg.system, impurity_config=impurity_config)
